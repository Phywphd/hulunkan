...
"""
码头护轮坎闯入检测 - 多模型对比测试脚本
功能：调用 Logic A/B 进行图像分类，并统计 Alarm/Normal 数量。
"""
...
def run_ensemble_test(models_dict, logic_func):
    """
    运行集成决策测试：采用 'OR' 策略以最大化召回率
    """
    # 1. 预加载所有模型以提高效率
    print("正在初始化集成模型库...")
    ensemble = {}
    for name, config in models_dict.items():
        if config["type"] == "rtdetr":
            m = RTDETR(config["path"]) if config["type"] == "rtdetr" else YOLO(config["path"])
        elif config["type"] == "yolo":
            m = YOLO(config["path"])
            m = fix_model_attn(m)
        elif config["type"] == "deim":
            m = DEIMWrapper(config["config"], config["path"])
    ...
    # 2. 处理正样本 (Alarm Images)
    print(f"集成决策：正在处理报警图像 ({len(alarm_imgs)}张)...")
    start_time = time.perf_counter()
    for img_p in alarm_imgs:
        # 只要任意一个模型报警，结果就是报警
        is_alarm_any = any([logic_func(img_p, m)[0] for m in ensemble.values()])
        if is_alarm_any:
            tp += 1
        else:
            fn += 1

    # 3. 处理负样本 (Normal Images)
    print(f"集成决策：正在处理正常图像 ({len(normal_imgs)}张)...")
    for img_p in normal_imgs:
        is_alarm_any = any([logic_func(img_p, m)[0] for m in ensemble.values()])
        if is_alarm_any:
            fp += 1
        else:
...
def main():
    ...
    # 运行各个独立模型的测试
    all_stats = []
    current_logic = logic_C if LOGIC == 'C' else (logic_B if LOGIC == 'B' else logic_A)

    # 运行集成测试
    ensemble_stat = run_ensemble_test(MODELS, current_logic)
    all_stats.append(ensemble_stat)

    # --- 输出展示 ---
    print("\n" + "=" * 130)
    print(f"{'模型/决策方式':<20} | {'TP':<6} | {'FN':<6} | {'FP':<6} | {'TN':<6} | {'Acc':<10} | {'Pre':<10} | {'Rec':<10} | {'F1'} | {'Speed(ms)':<10}")
    print("-" * 130)
    for s in all_stats:
        print(f"{s['name']:<20} | {s['tp']:<6} | {s['fn']:<6} | {s['fp']:<6} | {s['tn']:<6} | "
              f"{s['acc']:<10.4f} | {s['pre']:<10.4f} | {s['rec']:<10.4f} | {s['f1']:<8.4f} | {s['speed']:<10.2f}")
    print("=" * 130)
